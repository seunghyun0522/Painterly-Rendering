...
//paint function 에서 불러오는 기본적인 procedure paintLayer(canvas, referenceImage, R) 

void paintLayer(IplImage* canvas, IplImage* reference, int R, int mode) {
	...
	for (int y = grid / 2; y <= size.height; y += grid) { // y값을 grid 값을 기준으로 반복함
		for (int x = grid / 2; x <= size.width; x += grid) { // x값을 grid 값을 기준으로 반복함
			...
			M_x1 = x - grid / 2; M_x2 = x + grid / 2; 
			M_y1 = y - grid / 2; M_y2 = y + grid / 2; 

			// 구역 M이 이미지의 범위를 넘어갈 수 있기 때문에 예외처리
			if (M_x2 > size.width) M_x2 = size.width;
			if (M_y2 > size.height) M_y2 = size.height;

			double areaError = 0; // 색칠할 best location을 찾기 위한 변수 (논문 상의 areaError)
...
void paint(IplImage* src, int* R, int R_length, int mode) {
	...
	//canvas := a new constant color image => 효과를 넣을 canvas를 저장하는 과정 
	IplImage* canvas = cvCreateImage(size, 8, 3); 
	cvSet(canvas, cvScalar(0, 0, 0)); // 초기 설정 캔버스를 검은색으로 정리

	// 가우시안 필터를 적용시킬 이미지를 미리 설정(논문상의 referenceImage)
	...
	cvSaveImage("c://temp//result.jpeg", canvas);
...
int main() {
	...
	while (1) { // 파일명을 입력받는 부분
		...
		if (src != 0)
			...
			printf("경로가 틀렸거나 파일이 존재하지 않습니다. \n");
	}

	int mode = 0; 


	// mode를 설정하여 0 일때는 circle, 1 일때는 stroke 모드로 들어간다. 

	while (1) { 
		printf("Press Number.... Circle Mode  [0]  or Stroke Mode  [1]  : ");
		scanf("%d", &mode); 
		if (mode == 0 || mode == 1)	break;
		else printf("-_- 0,1 을 입력하고 엔터해주세요!\n");

	}

	// 브러쉬 사이즈를  임의로 저장한다 (논문 상의 R1,R2,R3..., Ri)
	int R[] = { 32, 16, 8, 4, 2 }; 
